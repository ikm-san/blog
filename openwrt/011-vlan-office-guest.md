...
| 方法 | 向く用途 |
|---|---|
| SSID分離＋firewall | ゲストWi-FiとLANの分離（無線のみ） |
| VLAN | 有線機器（POS・カメラ）も含めた分離 |
...
| ネットワーク | 用途 | IPアドレス範囲 | VLAN ID |
|---|---|---|---|
| Staff | 業務PC・管理端末・NAS | 192.168.1.0/24 | 1（タグなし/デフォルト） |
| Guest | ゲストWi-Fi | 192.168.2.0/24 | 10 |
| Camera | 防犯カメラ・録画機 | 192.168.3.0/24 | 20 |
...
| Guest | Staff | **拒否** |
| Guest | Camera | **拒否** |
| Camera | WAN | 要件次第（録画機のクラウド接続のみ許可など） |
| Camera | Staff | **拒否** |
