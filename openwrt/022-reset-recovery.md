...
> Original note.com article: [リセットと復旧: 初期化前に確認すること【OpenWrt集中連載022】](https://note.com/ikmsan/n/n5088d68a2205)
...
| リセット種別 | 操作 | 結果 | バックアップが必要か |
|---|---|---|---|
| ソフトリセット（LuCI） | LuCIから実行 | 全設定を初期化 | はい |
| ハードリセット（ボタン） | 本体リセットボタン長押し | 全設定を初期化 | はい |
| 設定ファイルのロールバック | SSHでバックアップファイルをコピー | 変更前の設定に戻す | バックアップファイルが必要 |
| バックアップ復元 | LuCI/sysupgrade -r | バックアップ時点の全設定に戻す | はい |
...
| 症状 | 原因 | 対処 |
|---|---|---|
| `192.168.1.1` に接続できない | LAN IPを変更した | 変更後のIPアドレスでアクセス |
| パスワードが通らない | パスワードを変更した後に忘れた | ハードリセット実行 |
| Wi-Fi接続後にLuCIにアクセスできない | ゲストWi-FiはLuCIへのアクセスをブロック | 有線LANで接続し直す |
| ログインできるがWeb画面が表示されない | LuCIパッケージが壊れている | SSHからlighttpd再起動 |
