...
| 構成 | 用途 | LN6001-JPの役割 |
|---|---|---|
| ルーターモード（デフォルト） | LN6001-JPが1台でルーター・AP兼務 | WAN←→LAN変換・DHCP |
| AP Bridge | 既存ルーター配下へLN6001-JPをWi‑Fi APとして追加 | 親ルーターと同一サブネットのAP |
| WDS子機 | 親SSIDへ無線接続し、LN6001-JPを子機側APとして使う | 無線バックホール用子機 |
...
| 設定項目 | 値 |
|---|---|
| SSID | アクセスポイントとして出したいSSID |
| SSID Password | そのSSIDのパスワード |
| Gateway IP | 親ルーターまたはホームゲートウェイのIPアドレス |
| Access Point IP | LN6001-JPに割り当てる同一サブネット内のIPアドレス |
...
| 設定項目 | 値 |
|---|---|
| Parent SSID | 親ルーターのSSID |
| Parent SSID Password | 親SSIDのパスワード |
| Parent IP Address | 親ルーターのIPアドレス |
| Child IP Address | LN6001-JP子機に割り当てる同一サブネット内のIPアドレス |
...
wifi status
iw dev
iw dev <wlan名> link 2>/dev/null || true
...
| 症状 | 原因 | 対処 |
|---|---|---|
| AP Bridge後にLuCIへアクセスできない | 旧IPでアクセスしている | Setup Assistantで指定したAccess Point IPへアクセス |
| AP Bridgeで通信できない | Gateway IPまたはAccess Point IPが親ルーターサブネットと合っていない | 親ルーターIPと同一サブネットか確認 |
| WDS子機がつながらない | Parent SSIDまたはパスワードが違う | 親SSIDと暗号化方式を確認して再入力 |
| Setup AssistantがServicesに出ない | ブラウザ表示が更新されていない | ページ全体をリロード、またはブラウザを開き直す |
