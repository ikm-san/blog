...
| 方式 | 向く状況 | 注意点 |
|---|---|---|
| WireGuard（自前） | 固定IP・ポート開放が可能、経路を自分で管理したい | 固定IPとポート開放の条件を事前確認 |
| Tailscale | 固定IPなし、IPv4 over IPv6環境、複数端末の管理を楽にしたい | 外部サービスへの依存（Tailscaleのアカウント） |
| OpenVPN | 既存システムとの互換性が必要 | 速度はWireGuardより劣ることが多い |
...
tailscale ping <接続先のTailscaleノード名>
...
| 用途 | 推奨設定 |
|---|---|
| 自宅NASだけ | WireGuard AllowedIPs を NASのIPのみに絞る |
| 管理画面だけ | AllowedIPs を `192.168.1.1/32` |
| LAN全体 | `192.168.1.0/24` |
| 複数ネットワーク | VLANの各セグメントをAllowedIPsに追加 |
