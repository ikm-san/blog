...
LN6001-JPはOpenWrtベースなので、`luci-app-adblock` を使ってLuCIから管理できます。

ただし、パッケージ導入、ブロックリスト追加、LAN側DNS配布、Force Local DNS、自動更新まで手で整えると、初回は少し手順が多くなります。自分で細かく組むのが難しい場合は、動作確認済みの導入スクリプトを使うほうが入りやすいです。

一方、最初から大量のブロックリストを入れすぎると、誤ブロックでWebサイトやアプリが正常に動かなくなることがあります。まずは動く状態を作り、必要に応じてホワイトリストやブロックリストを調整するほうが扱いやすいです。

また、DNS広告ブロックは「すべての広告を完全に消す機能」ではありません。アプリ内広告やDoH（DNS over HTTPS）を使う端末には効きにくいこともあります。

この記事では、Adblock導入スクリプトを使う方法、手動で設定する方法、誤ブロック対応、DoH迂回への考え方を、家庭・小規模オフィス向けに順番に整理します。
...
1. バックアップを取る
2. 導入スクリプト、または手動で `adblock` と `luci-app-adblock` を入れる
3. 普段使うサイトやアプリで誤ブロックがないか確認する
...
Adblockを手動で入れて、ブロックリスト、LAN側DNS配布、Force Local DNS、自動更新まで自分で整えるのが大変なら、以下のGitHubで公開している広告ブロック導入スクリプトを使う方法があります。

このスクリプトはLN6001-JPで動作確認した実用向けの導入ルートです。最初から全部を手で組むより、まず動く状態を作ってから中身を理解していくほうが入りやすい人も多いです。

- スクリプト: https://github.com/ikm-san/velop
- 内容: AdblockとLuCI用パッケージ、日本語LuCI表示、TOFUフィルター、LAN向けDNS設定、Force Local DNS、自動更新cronなどをまとめて設定する
- 効果の目安: ブラウザ上の広告表示を大きく減らし、ルーター配下のスマホやPC全体に効かせやすくする
- 向く場面: 家庭や小規模オフィスで広告を減らしたい場合、ビジネスや教育の現場で不要な広告を減らしたい場合、子ども用端末のゲーム・マンガ広告を極力グレーアウトしたい場合
...
- `opkg update` とパッケージ導入をリトライ付きで実行
- `adblock`、`luci-app-adblock`、`luci-i18n-adblock-ja` をインストール
- TOFUフィルターをAdblockのソースへ追加
- LANのDHCPで、ルーター自身のIPv4/IPv6アドレスをDNSとして配布
- AdblockのDNS backendを `dnsmasq` にし、Force Local DNSを有効化
- Adblockを有効化・再起動し、`dnsmasq` も再起動
- `t.co` を許可リストへ追加
- 子ども向け用途として一部ドメインをブロックリストへ追加
- 毎日3:50にAdblockをリロードするcronを追加
- 最後に再起動するか確認

curl実行前に、GitHub上のREADMEやスクリプト内容を確認してから進めることをおすすめします。

スクリプトはルーター設定を変更し、最後に再起動確認も行います。特に、既存の `dhcp.lan.dhcp_option` と `dhcp.lan.dns` を整理してLAN向けDNS配布を入れ直すため、独自DNS設定をしている場合は必ずバックアップを取ってから実行してください。
...
ここから下は、スクリプトを使わずに手動で設定したい人向けです。まず早く動かしたい場合は、上の導入スクリプトを使ってから、必要なところだけ調整する流れでも問題ありません。
...
**対応策（一例）:**
導入スクリプトはForce Local DNSを有効化しますが、端末がDoHで外部へ暗号化DNSを使う場合、それだけでは完全に防げないことがあります。
...
期待値を合わせた上で、セットアップが難しい場合は導入スクリプトを使い、動き始めてからホワイトリストやブロックリストを調整するほうが運用しやすくなります。
