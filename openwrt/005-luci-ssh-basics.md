...
| LuCIで見る場所 | SSHで見るコマンド | 何が分かるか |
|---|---|---|
| Status → Overview | `ubus call system board` / `uptime` / `free` | ファームウェア、稼働時間、メモリ |
| Network → Interfaces | `ifstatus wan` / `ifstatus wan6` | WAN/WAN6の接続状態 |
| Network → Wireless | `wifi status` / `iw dev` | Wi-Fi radio、SSID、無線IF |
| Network → DHCP and DNS | `cat /tmp/dhcp.leases` / `uci show dhcp` | DHCPリース、DNS/DHCP設定 |
| Network → Firewall | `uci show firewall` / `iptables -L -n -v` | firewall設定と実適用状態 |
| Status → System Log | `logread | tail -n 80` | 直近ログ |
...
| ファイル | 管理する設定 |
|---|---|
| `/etc/config/network` | WAN、LAN、VLAN、IPアドレス |
| `/etc/config/wireless` | Wi-FiのSSID、暗号化、無線設定 |
| `/etc/config/firewall` | zone、転送ルール、ポート開放 |
| `/etc/config/dhcp` | DHCP、DNS、固定割り当て |
| `/etc/config/system` | ホスト名、時刻、管理系 |
