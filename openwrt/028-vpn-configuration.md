...
| 条件 | 推奨方式 |
|---|---|
| プロバイダーから固定IPを付与されている | WireGuard |
| HGW/ONUからポート開放（UDPポート転送）が可能 | WireGuard |
| IPoE（IPv4 over IPv6）でポート開放が困難 | Tailscale |
| 固定IPなし・動的IPのみ | Tailscale |
| 接続端末が多く（5台以上）管理を楽にしたい | Tailscale |
| 外部サービスへの依存をゼロにしたい | WireGuard |
...
| 用途 | WireGuard AllowedIPs / Tailscale ACL設定 |
|---|---|
| NASだけアクセスしたい | `192.168.1.100/32` |
| LAN全体にアクセスしたい | `192.168.1.0/24` |
| 複数VLANにアクセスしたい | `192.168.1.0/24, 192.168.2.0/24` |
| 全通信をVPN経由にする（完全トンネル） | `0.0.0.0/0` |
