...
| ゾーン | Input | Output | Forward | 用途 |
|---|---|---|---|---|
| lan | ACCEPT | ACCEPT | ACCEPT | 内部LAN（信頼する端末） |
| wan | REJECT | ACCEPT | REJECT | インターネット側 |
...
| Output | `ACCEPT` |
| Forward | `REJECT` |
| Covered networks | `guest`（作成済みのネットワーク） |
...
| Source zone | `guest` |
| Destination zone | `Device（Router itself）` |
