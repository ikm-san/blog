...
| 回線パターン | 初期設定の考え方 |
|---|---|
| DHCP自動でつながる回線 | 工場出荷時設定で開始できる場合が多い |
| ひかり電話HGWあり | HGWがIPoEを担当し、LN6001-JPはDHCP自動で動く場合がある |
| ひかり電話HGWなしのNTT IPoE | Linksys公式のオートIPoEモジュールを使う |
| PPPoE | LuCIのWAN設定でPPPoE情報を入力する |
| 固定IP / IPIP方式 | 契約情報に合わせて公式手順を確認する |
...
| 項目 | 内容 |
|---|---|
| 管理画面URL | `https://192.168.1.1` など |
| 管理ユーザー名 | `root` |
| 管理パスワード | パスワードマネージャーに保管 |
| 2.4GHz SSID | 設定した名前 |
| 5GHz SSID | 設定した名前 |
| 6GHz SSID | 設定した名前 |
| Wi-Fiパスワード | パスワードマネージャーに保管 |
| LAN側IPアドレス | `192.168.1.1` など |
| WAN接続方式 | DHCP/PPPoE/IPoEなど |
| プロバイダ名 | 契約内容を確認 |
| IPoEサービス名 | OCNバーチャルコネクト/transixなど |
| 追加したモジュール | IPoEモジュールなど |
| 初期設定日 | YYYY-MM-DD |
