...
| 帯域 | 向く端末 | 特徴 |
|---|---|---|
| 2.4GHz | IoT、古い端末、遠い部屋 | 届きやすいが混雑しやすい |
| 5GHz | PC、スマートフォン、テレビ | 日常の主力、バランスがよい |
| 6GHz | Wi-Fi 7対応端末 | 混雑少なく高速、対応端末が必要 |
...
| フィールド | 設定内容 |
|---|---|
| ESSID | Wi-Fi名（例: `MyHome` / `MyHome_5G` / `MyHome_6G`） |
| Mode | `Access Point`（変更不要） |
| Network | 接続するLAN/networkを選択 |

4. **Wireless Security** タブ:

| フィールド | 設定内容 |
|---|---|
| Encryption | `WPA2-PSK`（互換性重視）または `WPA3-SAE`（最新端末向け） |
...
| SSID | 帯域 | 用途 | 暗号化 |
|---|---|---|---|
| `MyHome` | 5GHz | PC・スマートフォン | WPA3-SAE |
| `MyHome_IoT` | 2.4GHz | IoT機器 | WPA2-PSK |
| `MyHome_Guest` | 5GHz | 来客端末 | WPA2-PSK |

### Wi-Fi 7 MLO構成（対応端末あり）

| SSID | 帯域 | 用途 | 暗号化 |
|---|---|---|---|
| `MyHome` | 2.4GHz | MLO（全帯域共通SSID） | WPA3-SAE |
| `MyHome` | 5GHz | MLO（全帯域共通SSID） | WPA3-SAE |
| `MyHome` | 6GHz | MLO（全帯域共通SSID） | WPA3-SAE |
| `MyHome_IoT` | 2.4GHz | IoT機器 | WPA2-PSK |
| `MyHome_Guest` | 5GHz | 来客端末 | WPA2-PSK |
...
| SSID | 帯域 | 用途 | 暗号化 |
|---|---|---|---|
| `Staff_WiFi` | 5GHz | 業務PC・スタッフ端末 | WPA3-SAE |
| `Guest_WiFi` | 5GHz | 来客・来訪者 | WPA2-PSK |
| `IoT_Camera` | 2.4GHz | 防犯カメラ・設備 | WPA2-PSK |
