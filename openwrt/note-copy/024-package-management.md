...
adblock、tcpdump、監視ツールなどを追加できる一方で、ストレージ容量や依存関係、ファームウェア更新時の再インストールも意識する必要があります。
...
# パッケージを検索
opkg list | grep <キーワード>
opkg list | grep adblock
...
overlayfs:/overlay 100.0M  60.0M  40.0M  60% /
```
...
![表 01](https://raw.githubusercontent.com/ikm-san/blog/main/openwrt/assets/024/table-01.png)

最初は、adblock や tcpdump のような「用途が明確なもの」から追加するほうが安全です。
...
# adblockとLuCI用UIをインストール
opkg install adblock luci-app-adblock

# インストール確認
opkg list-installed | grep adblock
opkg files adblock | head
...
最初は「インストールできたか」「LuCIへメニューが出るか」を確認できれば十分です。
...
# 戻したあとに確認
opkg list-installed | grep -E 'adblock|luci-app-adblock'
/etc/init.d/adblock status
```

最初は「Adblock」「VPN」「IPoE関連」が戻るかを優先確認すると整理しやすくなります。

`opkg` は便利ですが、「普通のLinux PC」と同じ感覚で大量導入すると整理しにくくなることがあります。

## opkgの注意事項

![表 02](https://raw.githubusercontent.com/ikm-san/blog/main/openwrt/assets/024/table-02.png)
...
# 削除後の確認
opkg list-installed | grep <パッケージ名> || true
df -h
logread | tail -n 80
```
...
3. adblock や tcpdump など用途が明確なものから追加する
