...
| グループ | 端末例 | 目的 |
|---|---|---|
| 親/大人 | PC・スマートフォン | 制限なし、通常LAN |
| 子ども | タブレット・ゲーム機 | フィルタリングDNS、時間制限（端末側） |
| IoT | スマート家電・カメラ・スピーカー | LAN内通信を制限 |
| 来客 | 友人の端末 | インターネットのみ |
...
| ネットワーク | IPアドレス範囲 | SSID例 | DNS |
|---|---|---|---|
| LAN（親用） | 192.168.1.0/24 | `MyHome` | 通常DNS |
| Kids | 192.168.3.0/24 | `MyHome_Kids` | フィルタリングDNS |
| IoT | 192.168.4.0/24 | `MyHome_IoT` | 通常DNS（または制限DNS） |
| Guest | 192.168.2.0/24 | `MyHome_Guest` | 通常DNS |
...
| サービス | DNS1 | DNS2 | 特徴 |
|---|---|---|---|
| Cloudflare Family | `1.1.1.3` | `1.0.0.3` | マルウェア・成人向けコンテンツをフィルタ |
| Cloudflare Malware | `1.1.1.2` | `1.0.0.2` | マルウェアのみフィルタ |
| Google SafeSearch | `8.8.8.8` | `8.8.4.4` | SafeSearch強制（Googleのみ） |
| OpenDNS Family | `208.67.222.123` | `208.67.220.123` | 細かいカテゴリ設定が可能 |
...
| 項目 | DNSフィルタリングでできる？ | 補足 |
|---|---|---|
| 危険サイトや成人向けサイトの入口を減らす | できる | フィルタリングDNSが得意な領域 |
| 子ども用端末に別DNSを配る | できる | DHCP で Kids 用に配り分けできる |
| 利用時間の制限 | できないことが多い | 端末側の機能や別の制御が必要 |
| アプリ単位の制御 | 難しい | アプリ内通信や直接IP通信はDNSを通らないことがある |
| DoH での回避を完全に防ぐ | 難しい | 端末側設定も影響する |
| YouTubeやSNSを細かく制限する | 難しい | 同じドメイン配下に複数用途が混在しやすい |
